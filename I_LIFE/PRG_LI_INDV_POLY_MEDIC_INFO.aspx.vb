...
Partial Class I_LIFE_PRG_LI_INDV_POLY_MEDIC_INFO
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        strTableName = "TBIL_POLICY_MEDIC_EXAM"
        ...
        If Not (Page.IsPostBack) Then
            ...
            Call gnProc_Populate_Box("IL_CODE_LIST_UND", "003", Me.cboMed_Exam_code)
            Call gnProc_Populate_Box("IL_CODE_LIST_UND", "008", Me.cboMed_Clinic_Code)
    ...
    Private Sub Proc_DataBind()
        ...
        strSQL = strSQL & " ,CASE WHEN PT.TBIL_POL_MED_STATUS IN('P') THEN 'Pending'"
        strSQL = strSQL & "       WHEN PT.TBIL_POL_MED_STATUS IN('S') THEN 'Successful'"
        strSQL = strSQL & "       WHEN PT.TBIL_POL_MED_STATUS IN('U') THEN 'Unsuccessful'"
        strSQL = strSQL & "       ELSE '*'"
        strSQL = strSQL & " END AS TBIL_MED_STATUS_NAME"

        strSQL = strSQL & " ,MED_003.TBIL_COD_LONG_DESC AS TBIL_MED_EXAM_NAME"
        strSQL = strSQL & " ,MED_008.TBIL_COD_LONG_DESC AS TBIL_MED_CLINIC_NAME"
        strSQL = strSQL & " FROM " & strTable & " AS PT "

        strSQL = strSQL & " LEFT JOIN TBIL_LIFE_CODES AS MED_003"
        strSQL = strSQL & " ON (MED_003.TBIL_COD_ITEM = PT.TBIL_POL_MED_CODE"
        strSQL = strSQL & " AND MED_003.TBIL_COD_TAB_ID = 'L01'"
        strSQL = strSQL & " AND MED_003.TBIL_COD_TYP = '003')"
        strSQL = strSQL & " LEFT JOIN TBIL_LIFE_CODES AS MED_008"
        strSQL = strSQL & " ON (MED_008.TBIL_COD_ITEM = PT.TBIL_POL_MED_CLINIC_CD"
        strSQL = strSQL & " AND MED_008.TBIL_COD_TAB_ID = 'L01'"
        strSQL = strSQL & " AND MED_008.TBIL_COD_TYP = '008')"

        strSQL = strSQL & " WHERE PT.TBIL_POL_MED_FILE_NO = '" & RTrim(strF_ID) & "'"
        strSQL = strSQL & " AND PT.TBIL_POL_MED_PROP_NO = '" & RTrim(strQ_ID) & "'"
        strSQL = strSQL & " ORDER BY TBIL_POL_MED_REC_ID"
    ...
    Private Sub Proc_DoDelete()
        ...
        strSQL = strSQL & " WHERE TBIL_POL_MED_FILE_NO = '" & RTrim(strREC_ID) & "'"
        strSQL = strSQL & " AND TBIL_POL_MED_PROP_NO = '" & RTrim(Me.txtQuote_Num.Text) & "'"
    ...
    Protected Sub Proc_DoDelete_Record()
        ...
        strSQL = strSQL & " WHERE TBIL_POL_MED_FILE_NO = '" & RTrim(strREC_ID) & "'"
        strSQL = strSQL & " AND TBIL_POL_MED_PROP_NO = '" & RTrim(Me.txtQuote_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_POL_MED_REC_ID = " & Val(RTrim(Me.txtRecNo.Text)) & ""
    ...
    Private Sub Proc_DoSave()
        ...
        Call MOD_GEN.gnGET_SelectedItem(Me.cboMed_Exam_code, Me.txtMed_Exam_Code, Me.txtMed_Exam_CodeName, Me.lblMsg)
        If Me.txtMed_Exam_Code.Text = "" Then
            Me.lblMsg.Text = "Missing " & Me.lblMed_Exam_Code.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If

        If RTrim(Me.txtMed_Exam_Date.Text) = "" Or Len(Trim(Me.txtMed_Exam_Date.Text)) <> 10 Then
            Me.lblMsg.Text = "Missing or Invalid date - " & Me.lblMed_Exam_Date.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If

        'Validate date
        myarrData = Split(Me.txtMed_Exam_Date.Text, "/")
        If myarrData.Count <> 3 Then
            Me.lblMsg.Text = "Missing or Invalid " & Me.lblMed_Exam_Date.Text & ". Expecting full date in ddmmyyyy format ..."
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If
        ...
        If blnStatusX = False Then
            ...
            Exit Sub
        End If
        Me.txtMed_Exam_Date.Text = RTrim(strMyDte)
        ...
        Call MOD_GEN.gnGET_SelectedItem(Me.cboMed_Clinic_Code, Me.txtMed_Clinic_Code, Me.txtMed_Clinic_CodeName, Me.lblMsg)
        If Me.txtMed_Clinic_Code.Text = "" Then
            Me.lblMsg.Text = "Missing " & Me.lblMed_Clinic_Code.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If

        Call MOD_GEN.gnGET_SelectedItem(Me.cboMed_Status, Me.txtMed_Status_Code, Me.txtMed_Status_Name, Me.lblMsg)
        If Me.txtMed_Status_Code.Text = "" Then
            Me.lblMsg.Text = "Missing " & Me.lblMed_Sttus.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If
        ...
        strSQL = strSQL & " WHERE TBIL_POL_MED_FILE_NO = '" & RTrim(txtFileNum.Text) & "'"
        'strSQL = strSQL & " AND TBIL_POL_MED_PROP_NO = '" & RTrim(txtQuote_Num.Text) & "'"
        'If Val(LTrim(RTrim(Me.txtRecNo.Text))) <> 0 Then
        strSQL = strSQL & " AND TBIL_POL_MED_REC_ID = '" & Val(RTrim(txtRecNo.Text)) & "'"
        'End If
        ...
        Try
            ...
            If obj_DT.Rows.Count = 0 Then
                ...
                drNewRow("TBIL_POL_MED_FILE_NO") = RTrim(Me.txtFileNum.Text)
                drNewRow("TBIL_POL_MED_PROP_NO") = RTrim(Me.txtQuote_Num.Text)
                'drNewRow("TBIL_POL_MED_POLY_NO") = RTrim(Me.txtPolNum.Text)

                drNewRow("TBIL_POL_MED_CODE") = RTrim(Me.txtMed_Exam_Code.Text)

                If Trim(Me.txtMed_Exam_Date.Text) <> "" Then
                    drNewRow("TBIL_POL_MED_DATE") = Dte_Current
                End If

                drNewRow("TBIL_POL_MED_CLINIC_CD") = RTrim(Me.txtMed_Clinic_Code.Text)
                drNewRow("TBIL_POL_MED_STATUS") = RTrim(Me.txtMed_Status_Code.Text)

                drNewRow("TBIL_POL_MED_FLAG") = "A"
                drNewRow("TBIL_POL_MED_OPERID") = CType(myUserIDX, String)
                drNewRow("TBIL_POL_MED_KEYDTE") = Now
            ...
            Else
                ...
                With obj_DT
                    .Rows(0)("TBIL_POL_MED_FILE_NO") = RTrim(Me.txtFileNum.Text)
                    .Rows(0)("TBIL_POL_MED_PROP_NO") = RTrim(Me.txtQuote_Num.Text)
                    '.Rows(0)("TBIL_POL_MED_POLY_NO") = RTrim(Me.txtPolNum.Text)

                    .Rows(0)("TBIL_POL_MED_CODE") = RTrim(Me.txtMed_Exam_Code.Text)

                    If Trim(Me.txtMed_Exam_Date.Text) <> "" Then
                        .Rows(0)("TBIL_POL_MED_DATE") = Dte_Current
                    End If

                    .Rows(0)("TBIL_POL_MED_CLINIC_CD") = RTrim(Me.txtMed_Clinic_Code.Text)
                    .Rows(0)("TBIL_POL_MED_STATUS") = RTrim(Me.txtMed_Status_Code.Text)


                    .Rows(0)("TBIL_POL_MED_FLAG") = "C"
                    '.Rows(0)("TBIL_POL_NED_OPERID") = CType(myUserIDX, String)
                    '.Rows(0)("TBIL_POL_MED_KEYDTE") = Now
                End With
    ...
    Private Function Proc_DoOpenRecord(ByVal FVstrGetType As String, ByVal FVstrRefNum As String, Optional ByVal FVstrRecNo As String = "", Optional ByVal strSearchByWhat As String = "FILE_NUM") As String
        ...
        strTable = strTableName
        strSQL = ""
        strSQL = strSQL & "SELECT TOP 1 MED_TBL.*"
        strSQL = strSQL & " FROM " & strTable & " AS MED_TBL"
        strSQL = strSQL & " WHERE MED_TBL.TBIL_POL_MED_FILE_NO = '" & RTrim(strREC_ID) & "'"
        If Val(LTrim(RTrim(FVstrRecNo))) <> 0 Then
            strSQL = strSQL & " AND MED_TBL.TBIL_POL_MED_REC_ID = '" & Val(FVstrRecNo) & "'"
        End If
        'strSQL = strSQL & " AND PT.TBIL_POLY_PROPSAL_NO = '" & RTrim(strQ_ID) & "'"
        'strSQL = strSQL & " AND PT.TBIL_POLY_POLICY_NO = '" & RTrim(strP_ID) & "'"

        strSQL = "SPIL_GET_POLICY_MED_EXAM"
        ...
        If (objOLEDR.Read()) Then
            ...
            Me.txtFileNum.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_FILE_NO") & vbNullString, String))
            'Call Proc_DDL_Get(Me.ddlGroup, RTrim(Me.txtGroupNum.Text))
            Me.txtRecNo.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_REC_ID") & vbNullString, String))

            Me.txtQuote_Num.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_PROP_NO") & vbNullString, String))
            Me.txtPolNum.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_POLY_NO") & vbNullString, String))

            Me.txtMed_Exam_Code.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_CODE") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboMed_Exam_code, RTrim(Me.txtMed_Exam_Code.Text))
            Me.txtMed_Exam_CodeName.Text = RTrim(CType(objOLEDR("TBIL_MED_EXAM_NAME") & vbNullString, String))

            If IsDate(objOLEDR("TBIL_POL_MED_DATE")) Then
                Me.txtMed_Exam_Date.Text = Format(CType(objOLEDR("TBIL_POL_MED_DATE"), DateTime), "dd/MM/yyyy")
            End If

            Me.txtMed_Clinic_Code.Text = RTrim(CType(objOLEDR("TBIL_POL_MED_CLINIC_CD") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboMed_Clinic_Code, RTrim(Me.txtMed_Clinic_Code.Text))
            Me.txtMed_Clinic_CodeName.Text = RTrim(CType(objOLEDR("TBIL_MED_CLINIC_NAME") & vbNullString, String))
